...
void output (std::map<std::string, std::pair<int, std::set<int>>> &wordMap, std::map<std::string, int> &link)
{
  ...
  out << std::endl;
  out << "------------------------------------------------\n";
  out << std::left << std::setw(20) << "WORD" << std::left << std::setw(20) << "FREQUENCY" <<  "LINES" << std::endl;
  for (auto i : wordMap)
  {
      if (i.second.first > 1)
      {
        out << std::left << std::setw(20) << i.first << std::left << std::setw(20) << i.second.first;
  ...
  {
    out << std::left << std::setw(100) << it->first << " in the line number " << it->second << std::endl;
  }
  out.close();
  system ("cls");
}
